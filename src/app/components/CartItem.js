...
const CartItem = ({ pizza }) => {
  ...
  return (
    <div className="select-none border-b border-white/10 last:border-0 pb-4">
      <div className="flex gap-x-4 mb-2">
        ...
        <div className="flex-1 flex flex-col gap-y-10">
          <div className="text-lg capitalize font-bold text-white">{pizza.name}</div>
          <div className="flex flex-col gap-y-1">
            <div className="capitalize font-medium text-[15px] text-white/70">{pizza.crust} crust</div>
            <div className="capitalize mb-2 font-medium text-[15px] text-white/70">{pizza.size} size</div>
            <div className="flex items-center gap-x-1">
              ...
              <div className="font-semibold flex flex-1 max-w-[30px] justify-center items-center text-sm text-white">{pizza.amount}</div>
        ...
        <div className="flex flex-col justify-between">
          ...
          <div>
            <span className="text-[17px] font-medium font-robotoCondensed text-white">Rs. {parseFloat(pizza.price * pizza.amount).toLocaleString()}</span>
          </div>
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-3 mt-4">
        ...
        {pizza.additionalTopping.map((topping, index) => (
          <div className="capitalize text-xs bg-white/10 text-white font-medium px-3 py-1 rounded-full leading-none" key={index}>
            {topping.name}
          </div>
        ))}
      </div>
